...
def aggregate_mtslice(tdata):
    ...
    unique_pair = np.unique(
        np.vstack((tdata.mean_temp, tdata.daily_temp)).T, axis=0
    )
    ...
    return utils.TempData(
        mean_temp, daily_temp, obs_mean, obs_std, study_id, data_id
    )
    ...
    for i in range(tdata_mt.num_studies):
        ...
        if tdata_mt.study_sizes[i] >= 5:
            spline = xspline.XSpline(
                np.array([cov.min(), ref, cov.max()]), 2, l_linear=True
            )
        else:
...
def adjust_agg_std_mtslice(tdata_mt, ref=None):
    ...
    # fit the curve
    spline = xspline.XSpline(
        np.array([tdata_mt.daily_temp.min(), ref, tdata_mt.daily_temp.max()]),
        2,
        l_linear=True,
    )

    beta = utils.fit_spline(
        tdata_mt.obs_mean, tdata_mt.obs_std, tdata_mt.daily_temp, spline
    )

    residual = tdata_mt.obs_mean - spline.design_mat(tdata_mt.daily_temp).dot(
        beta
    )
...
def fit_trend(tdata, surface_result, inlier_pct=1.0):
    ...
    tdata_residual.obs_mean -= surface_result.surface_func(
        tdata.mean_temp, tdata.daily_temp
    )
    ...
    for i, mean_temp in enumerate(tdata.unique_mean_temp):
        ...
        (beta_at_mt, beta_var_at_mt, gamma_at_mt, random_effects_at_mt) = (
            fit_trend_mtslice(tdata_at_mean_temp, tmrl, inlier_pct=inlier_pct)
        )
    ...
    trend_result = utils.TrendResult(
        beta, beta_var, gamma, random_effects, tdata.unique_mean_temp
    )
    # import pdb; pdb.set_trace()
...
def fit_trend_mtslice(tdata_at_mean_temp, tmrl, inlier_pct=0.9, debug=False):
    ...
    if debug:
        print(
            "number of locations at mean temp", tdata_at_mean_temp.num_studies
        )
        outer_verbose = True
        inner_print_level = 5
    else:
    ...
    (beta, gamma, trimming_weights) = lt.fitModel(
...
def plot_mtslice(tdata, mt, ylim=None, ax=None, use_colors=True):
    ...
    if use_colors:
        ...
        for i in range(tdata_mt.num_studies):
            ax.scatter(
                tdata_mt.daily_temp[study_slices[i]],
                tdata_mt.obs_mean[study_slices[i]],
                s=1.0 / tdata_mt.obs_std[study_slices[i]],
            )
    else:
        ax.scatter(
            tdata_mt.daily_temp, tdata_mt.obs_mean, s=1.0 / tdata_mt.obs_std
        )
