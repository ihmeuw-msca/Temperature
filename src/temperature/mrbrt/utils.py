...
def normalizedDesignIMat(x0, x1, spline):
    ...
    mat[int_idx, :] = (
        spline.designIMat(x0[int_idx], x1[int_idx], 1) / dx[int_idx, :]
    )
...
class ratioInitIP:
    ...
    def initialize(self, beta0=None, print_level=0, max_iter=20):
        ...
        opt_problem = ipopt.problem(
            n=self.k, m=2 * self.N, problem_obj=self, cl=self.c[0], cu=self.c[1]
        )
...
def constructZCov(z_cov_list, spline_list=[]):
    ...
    return Z, Z_list, id_gamma_list, id_spline_gamma_list, k_gamma_list
...
def constructPrior(prior_list, mr):
    ...
    for prior in mr.prior_list:
        ...
        if (
            prior["prior_type"] == "spline_shape_function_uprior"
            or prior["prior_type"] == "spline_shape_function_gprior"
            or prior["prior_type"] == "spline_shape_derivative_uprior"
            or prior["prior_type"] == "spline_shape_derivative_gprior"
            or prior["prior_type"] == "spline_shape_monotonicity"
            or prior["prior_type"] == "spline_shape_convexity"
        ):
            x = np.linspace(
                prior["interval"][0], prior["interval"][1], prior["num_points"]
            )
        ...
        if (
            ...
            id_C_list.append(
                slice(num_constraints, num_constraints + prior["num_points"])
            )
        ...
        if (
            prior["prior_type"] == "spline_shape_function_gprior"
            or prior["prior_type"] == "spline_shape_derivative_gprior"
        ):
            id_H_list.append(
                slice(num_regularizers, num_regularizers + prior["num_points"])
            )
        ...
        if prior["prior_type"] == "ndspline_shape_function_uprior":
            ...
            x = [
                np.linspace(interval[i][0], interval[i][1], num_points[i])
                for i in range(spline.ndim)
            ]
            id_C_list.append(
                slice(num_constraints, num_constraints + np.prod(num_points))
            )
        ...
        if prior["prior_type"] == "ndspline_shape_function_gprior":
            ...
            id_H_list.append(
                slice(num_regularizers, num_regularizers + np.prod(num_points))
            )
        ...
        if prior["prior_type"] == "spline_shape_max_derivative_uprior":
            id_C_list.append(
                slice(num_constraints, num_constraints + spline.num_invls)
            )
        ...
        if prior["prior_type"] == "spline_shape_max_derivative_gprior":
            id_H_list.append(
                slice(num_regularizers, num_regularizers + spline.num_invls)
            )
        ...
        if prior["prior_type"] == "x_cov_constraint":
            id_C_list.append(
                slice(
                    num_constraints, num_constraints + prior["num_constraints"]
                )
            )
        ...
        if prior["prior_type"] == "x_cov_regularizer":
            id_H_list.append(
                slice(
                    num_constraints, num_constraints + prior["num_regularizers"]
                )
            )
        ...
        if prior["prior_type"] == "z_cov_constraint":
            ...
            gamma_id = slice(
                mr.k_beta + gamma_id.start, mr.k_beta + gamma_id.stop
            )
    ...
    if not C_list:
        ...
        def JC(x):
            ...
            for i in range(len(C_list)):
                mat[id_C_list[i], id_C_var_list[i]] = JC_list[i](
                    x[id_C_var_list[i]]
                )
    ...
    if not H_list:
        ...
        def JH(x):
            ...
            for i in range(len(H_list)):
                mat[id_H_list[i], id_H_var_list[i]] = JH_list[i](
                    x[id_H_var_list[i]]
                )
