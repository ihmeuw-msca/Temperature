...
def fit_surface(tdata, scale_params=[40.0, 1.25]):
    ...
    mr = mrbrt.MR_BRT(
        obs_mean, obs_std, study_sizes, x_cov_list, z_cov_list, spline_list
    )
    ...
    surface_result = utils.SurfaceResult(
        mr.beta_soln, beta_var, spline_list[0], scale_params=scale_params
    )
...
def fit_trend(tdata, surface_result, inlier_pct=1.0):
    ...
    for mean_temp in unique_mean_temp:
        (beta_at_mt, beta_var_at_mt, gamma_at_mt, random_effects_at_mt) = (
            fit_trend_at_mean_temp(
                residual_tdata, mean_temp, inlier_pct=inlier_pct
            )
        )
    ...
    trend_result = utils.TrendResult(
        beta, beta_var, gamma, random_effects, unique_mean_temp
    )
...
def sample_surface(
    mt, dt, num_samples, surface_result, trend_result, include_re=True
):
    # predict data
    ...
    mt_id = np.array(
        [
            np.where(trend_result.mean_temp == mt[i])[0][0]
