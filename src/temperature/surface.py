...
def fit_surface(tdata_agg, scale_params=[40.0, 1.25], linear_no_mono=False):
    ...
    for mt in mean_temp:
        ...
        daily_temp_range.append(
            [tdata_agg_mt.daily_temp.min(), tdata_agg_mt.daily_temp.max()]
        )
...
def plot_surface(tdata_agg, surface_result, num_levels=11):
    ...
    mean_temp = np.linspace(
        tdata_agg.mean_temp.min(), tdata_agg.mean_temp.max(), num_mt
    )
    daily_temp = np.linspace(
        tdata_agg.daily_temp.min(), tdata_agg.daily_temp.max(), num_dt
    )
